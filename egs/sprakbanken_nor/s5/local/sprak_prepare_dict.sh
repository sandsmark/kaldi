...
echo "Normalising "
python3 local/normalize_transcript_prefixed2.py local/norm_no/numbersUp.tbl data/train/text data/train/onlyids $dir/transcripts.tmp

# Additional normalisation, uppercasing, writing numbers etc.
# and recombine with 
local/norm_no/format_text.sh am $dir/transcripts.tmp > $dir/transcripts.am
cp $dir/transcripts.am data/train/onlytext
paste data/train/onlyids data/train/onlytext > data/train/text2 


# lmsents is output by sprak_data_prep.sh and contains
# sentences that are disjoint from the test and dev set 
python3 local/normalize_transcript.py local/norm_no/numbersUp.tbl data/local/data/lmsents $dir/lmsents.norm

echo "Waiting..."
wait

# Create wordlist from the AM transcripts
echo "Creating wordlist from AM transcripts"
...
local/norm_no/format_text.sh lm $dir/lmsents.norm > $dir/transcripts.txt
...
cp $dir/wlist.txt $dir/Wtemp_
for w in $dir/Wtemp_*; do
  (cat $w | espeak -q -vno -x > $w.pho) &
done
